...
;; TODO: Refine to check value for MODE is "development"
(when (env-value :mode)
  (format t "~& >> Updating service-status.lock file ~%")
  (with-open-file (stream "/tmp/service-status.lock" :direction :output :if-exists :supersede)
    (write "running" :stream stream))
  (format t "~& >> Updated service-status.lock file ~%"))
